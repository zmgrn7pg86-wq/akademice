...
async function findManualSource() {
  ...
  if (!query) {
    if (status) {
      status.hidden = false;
      status.textContent =
        "Kaynağı aramak için başlık, yazar, yıl veya DOI bilgilerinden en az birini yaz.";
    }
  ...
  if (status) {
    status.hidden = false;
    status.textContent =
      "Akademik veritabanında aranıyor. Bu işlem AI kredisi kullanmaz.";
  }
  ...
  try {
    ...
    if (status) {
      status.textContent =
        sources.length
          ? `${sources.length} olası akademik kayıt bulundu. Doğru kaynağı seç.`
    ...
    .forEach(
      (source) => {
        ...
        if (source.doi) {
          ...
          doiBadge.textContent =
            `DOI: ${source.doi}`;

          doiBadge.style.fontSize =
            "12px";
