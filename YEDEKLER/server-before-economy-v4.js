...
const PRIORITY_CONTEXT_RADIUS = 7000;
const MAX_PRIORITY_EXCERPTS = 8;
...
Aşağıda aynı belgenin parça analizleri ve yalnızca
yerel kurallarla seçilmiş kritik PDF metinleri
bulunmaktadır.
...
Başka araştırmalara ait yöntem, bulgu, sınırlılık veya
sonuçları bu araştırmanın bilgisi olarak alma.

Sınırlılık konusunda özellikle dikkatli ol.

Explicit sınırlılık:
Araştırmacının açıkça kendi araştırmasına ait bir
sınırlılık olarak belirttiği bilgidir.

Fact:
Belgede bulunan metodolojik gerçek.

Inference:
Academic AI tarafından yapılan akademik çıkarımdır.

Aşağıdaki ifadeleri otomatik sınırlılık kabul etme:
...
summary alanında amaç, problem, yöntem, örneklem,
veri toplama, analiz, bulgular ve sonuçları
bütünlüklü biçimde açıkla.
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      analysis =
        await recoverLimitationsIfNeeded(
          analysis,
          priority.excerpts,
          req.file.originalname
        );
      ...
      return res.json({
        ...
        meta: {
          ...
          longDocumentMode: true,

          limitationRecovery: true,
      ...
      let publicMessage =
        "Analiz sırasında beklenmeyen bir sorun oluştu.";
      ...
      if (
        ...
        publicMessage =
          "Academic AI bağlantısı doğrulanamadı.";
      } else if (
        error?.status === 429
      ) {
        publicMessage =
          "Academic AI şu anda yoğun. Lütfen tekrar deneyin.";
      }
...
`;
      ...
      let publicMessage =
        "AI Asistan şu anda yanıt oluşturamadı.";
...
app.listen(
  ...
  () => {
    ...
    console.log(
      "♻️ Sınırlılık ikinci kontrolü: AÇIK"
    );
