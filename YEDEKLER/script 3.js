...
buildResearchFramework
  ?.addEventListener(
    ...
    () => {
      ...
      // V7: Çerçeve hazırlanır hazırlanmaz yalnızca yerel depolamaya kaydet.
      // Burada ağır arayüz yeniden çizimi yapılmaz; sayfa geçişleri etkilenmez.
      const autoSavedProject = collectResearchProject();
      if (autoSavedProject.topic) {
        setResearchProject(autoSavedProject);
      }
...
// =====================================================
// ARAŞTIRMA PROJESİNİ HAFİF OTOMATİK KAYDET — V7
// YALNIZCA LOCALSTORAGE; ARAYÜZÜ YENİDEN ÇİZMEZ
// =====================================================

let researchAutoSaveTimerV7 = null;

function saveResearchProjectLightweight() {
  const project = collectResearchProject();
  if (!project.topic) return false;

  setResearchProject(project);
  return true;
}

function scheduleResearchAutoSaveV7() {
  clearTimeout(researchAutoSaveTimerV7);

  researchAutoSaveTimerV7 = setTimeout(() => {
    saveResearchProjectLightweight();
  }, 300);
}
...
saveResearchFramework
  ?.addEventListener(
    ...
    () => {
      ...
      // Kaydet butonunda da aynı kayıt anahtarını kullan.
      // Literatür sayfasına geçildiğinde kartlar otomatik yeniden çizilir.
...
function buildLiteratureResearchFit(item) {
  ...
  if (!project?.topic) {
    return {
      ...
      reason:
        "Bu kaynağın araştırmana uygunluğunu değerlendirmek için önce Araştırma bölümünde konunu kaydet.",
      thesisUse:
        "Araştırma konusu kaydedildiğinde Academic AI bu kartı otomatik günceller.",
      matchedTerms: [],
      evidenceLabel:
  ...
  const projectText = [
    ...
    ...(Array.isArray(project.keywords)
      ? project.keywords
      : []),
    project.turkishSearchTerms,
    project.englishSearchTerms
  ]
    .filter(Boolean)
    .join(" ");
  ...
  const hasRichAnalysis = Boolean(
    cleanValue(item?.purpose) ||
      cleanValue(item?.summary) ||
      cleanValue(item?.method) ||
      (Array.isArray(item?.findings) &&
  ...
  let level = "low";
  let title =
    "Uygunluk henüz net değil";

  if (matchedTerms.length >= 4) {
    level = "high";
    title =
      "Araştırmanla güçlü konu eşleşmesi var";
  } else if (
    matchedTerms.length >= 2
  ) {
    level = "medium";
    title =
      "Araştırmanla ilişkili görünüyor";
  } else if (
    matchedTerms.length === 1
  ) {
    level = "possible";
    title =
      "Araştırmanla olası bir bağlantı var";
  }

  const reason =
    matchedTerms.length
      ? `Ortak kavramlar: ${matchedTerms.join(
          ", "
        )}. Bu bir metadata/analiz alanı eşleşmesidir; kaynağın gerçek akademik uygunluğu içerik okunarak doğrulanmalıdır.`
      : "Başlık, anahtar kelimeler ve mevcut analiz alanlarında araştırma konunla belirgin bir ortak kavram bulunamadı. Kaynak yine de dolaylı veya kuramsal açıdan yararlı olabilir.";

  const usage = [];

  if (matchedTerms.length) {
    usage.push(
      `Literatür taramasında ${matchedTerms
        .slice(0, 3)
        .join(", ")} kavramlarıyla ilişkili çalışma olarak incelenebilir.`
    );
  }

  if (cleanValue(item?.method)) {
    usage.push(
      `Yöntem bölümünde ${shorten(
        cleanValue(item.method),
        90
      )} yaklaşımıyla karşılaştırma yapmak için kullanılabilir.`
    );
  }
  ...
  if (
    ...
    usage.push(
      "Tartışma bölümünde kendi bulgularınla karşılaştırılabilecek bir kaynak olabilir."
    );
  }

  if (!hasRichAnalysis) {
    usage.push(
      "Bu kayıtta bibliyografik metadata ağırlıklı bilgi var; tezde kullanmadan önce özetini veya PDF içeriğini analiz etmek gerekir."
    );
  }

  if (!usage.length) {
    usage.push(
      "Tezde hangi bölümde kullanılacağına karar vermek için önce kaynağın özet/PDF içeriğini incele."
    );
  }
  ...
  return {
    ...
    matchedTerms,
    evidenceLabel: hasRichAnalysis
      ? "Mevcut analiz alanları + metadata"
      : "Yalnızca metadata tabanlı ön değerlendirme"
  };
}

function createLiteratureResearchFitCard(
  item
) {
  const fit =
    buildLiteratureResearchFit(item);
  ...
  evidence.textContent =
    `Dayanak: ${fit.evidenceLabel}. AI kredisi kullanılmadı.`;
