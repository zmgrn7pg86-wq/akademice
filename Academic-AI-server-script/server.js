...
// =====================================================
// ÜCRETSİZ AKADEMİK KAYNAK ARAMASI
// OpenAlex metadata araması — AI kredisi kullanmaz
// Başlık / yazar / yıl / DOI ile çalışır.
// =====================================================

function cleanOpenAlexDoi(value) {
  return safeString(value)
    .replace(
      /^https?:\/\/(?:dx\.)?doi\.org\//i,
      ""
...
function normalizeOpenAlexSearchText(value) {
  return safeString(value)
    .toLocaleLowerCase("tr-TR")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
function getOpenAlexKeywords(work) {
  const keywordNames =
...
function getOpenAlexWorkType(work) {
  return safeString(
    work?.type_crossref ||
      work?.type
  );
...
function extractYearFromSourceSearch(value) {
  const text = safeString(value);
  const match =
    text.match(
      /(?:^|\s)(19\d{2}|20\d{2}|21\d{2})(?:\s|$)/
    );
...
function extractDoiFromSourceSearch(value) {
  const text = safeString(value);
  const match =
    text.match(
      /10\.\d{4,9}\/[-._;()/:A-Z0-9]+/i
  ...
  return match
    ? cleanOpenAlexDoi(match[0])
    : "";
}

function stripSourceSearchIdentifiers(
  value,
  year,
  doi
) {
...
function isLikelyReviewWork(source) {
  const title =
    normalizeOpenAlexSearchText(
      source?.title
    );
  ...
  return (
    /\bbook review\b/.test(title) ||
    /\breview of\b/.test(title) ||
    /\bkitap incelemesi\b/.test(title) ||
    /\bkitap degerlendirmesi\b/.test(title) ||
    type.includes("book review") ||
    type.includes("peer review")
  );
}

function scoreOpenAlexSource(
  source,
  criteria
) {
  let score = 0;
  ...
  if (expectedDoi) {
    ...
    if (
      ...
      score += 60;
      reasons.push("DOI eşleşti");
    } else {
      reasons.push("DOI eşleşmedi");
    }
  }
  ...
  if (criteria?.authors) {
    ...
    if (similarity >= 0.6) {
      reasons.push("Yazar eşleşti");
    } else if (
      similarity > 0
    ) {
      reasons.push(
        "Yazar kısmen eşleşti"
      );
    }
  }
  ...
  if (criteria?.year) {
    ...
    if (
      ...
      reasons.push("Yıl eşleşmedi");
    }
  }

  if (
    criteria?.genericQuery &&
    !criteria?.title
  ) {
    possible += 25;
  ...
  if (
    ...
    reasons.push(
      "İnceleme/değerlendirme kaydı olabilir"
    );
  }
...
function buildSourceSearchCriteria(body) {
  const query =
    safeString(
      body?.query
    );
  ...
  const inferredYear =
    extractYearFromSourceSearch(
      query
    );

  const inferredDoi =
    extractDoiFromSourceSearch(
      query
    );
...
function buildOpenAlexParams(
  ...
  if (criteria.year) {
    filters.push(
      `publication_year:${criteria.year}`
    );
  }
...
app.post(
  ...
  async (req, res) => {
    ...
    if (
      ...
      return res
        ...
        .json({
          success: false,
          error:
            "Arama için başlık, yazar, yıl veya DOI bilgilerinden en az biri gerekli.",
          message:
            "Arama için başlık, yazar, yıl veya DOI bilgilerinden en az biri gerekli."
        });
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return res
          ...
          .json({
            success: false,
            error:
              "Akademik kaynak veritabanından yanıt alınamadı.",
            message:
              "Akademik kaynak veritabanından yanıt alınamadı."
          });
      }
      ...
      console.log(
        `🔎 OpenAlex akıllı kaynak araması: "${
          criteria.query ||
          criteria.title ||
          criteria.authors ||
          criteria.year ||
          criteria.doi
        }" → ${sources.length} sonuç`
      );
      ...
      if (
        ...
        console.log(
          `📅 Yalnızca yıl modu: ${criteria.year} → yayın yılı filtresi kullanıldı.`
        );
      }
      ...
      console.error(
        "❌ OpenAlex kaynak arama hatası:",
        error
      );

      const publicMessage =
        isTimeout
          ? "Akademik kaynak araması zaman aşımına uğradı. Lütfen tekrar dene."
          : "Akademik kaynak araması şu anda tamamlanamadı.";
