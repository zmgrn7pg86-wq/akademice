...
// =====================================================
// ACADEMIC AI — YÖRÜNGEN KALEM YÜKLEME DENEYİMİ V15
// Arama, PDF analizi ve AI Asistan bekleme durumlarında kullanılır.
// Ek API çağrısı oluşturmaz.
// =====================================================

let orbitLoaderHideTimer = null;

function ensureOrbitLoader() {
  let overlay = document.getElementById("aaOrbitLoader");
  if (overlay) return overlay;

  overlay = document.createElement("div");
  overlay.id = "aaOrbitLoader";
  overlay.className = "aa-orbit-loader";
  overlay.dataset.stage = "working";
  overlay.setAttribute("role", "status");
  overlay.setAttribute("aria-live", "polite");
  overlay.setAttribute("aria-hidden", "true");

  overlay.innerHTML = `
    <div class="aa-orbit-loader-card">
      <div class="aa-orbit-loader-visual" aria-hidden="true">
        <svg viewBox="0 0 150 122">
          <ellipse class="aa-loader-shadow" cx="75" cy="108" rx="30" ry="5"></ellipse>
          <path class="aa-loader-orbit" d="M20 69c11-34 91-45 111-14 15 24-19 43-57 37"></path>
          <circle class="aa-loader-dot" cx="19" cy="69" r="4"></circle>
          <g class="aa-loader-pen-group">
            <path class="aa-loader-pen-body" d="M75 16l20 57-20 25-20-25z"></path>
            <path class="aa-loader-pen-nib" d="M75 16l5 48-5 13-5-13z"></path>
            <circle cx="75" cy="64" r="4" fill="#fffdf9"></circle>
            <circle class="aa-loader-eye" cx="68" cy="77" r="1.7"></circle>
            <circle class="aa-loader-eye" cx="82" cy="77" r="1.7"></circle>
            <path class="aa-loader-face" d="M70 84c3 3 7 3 10 0"></path>
          </g>
          <path class="aa-loader-spark" d="M30 28l2.5 6 6 2.5-6 2.5-2.5 6-2.5-6-6-2.5 6-2.5z"></path>
          <path class="aa-loader-spark" d="M120 20l1.6 4 4 1.6-4 1.6-1.6 4-1.6-4-4-1.6 4-1.6z"></path>
        </svg>
      </div>
      <span class="aa-orbit-loader-kicker">ACADEMIC AI · YÖRÜNGEN KALEM</span>
      <h3 class="aa-orbit-loader-title">Çalışıyoruz…</h3>
      <p class="aa-orbit-loader-detail">Bilgiler işleniyor.</p>
      <div class="aa-orbit-loader-track" aria-hidden="true">
        <div class="aa-orbit-loader-bar"></div>
      </div>
    </div>
  `;
...
function showOrbitLoader(stage = "working", title = "Çalışıyoruz…", detail = "Bilgiler işleniyor.") {
  clearTimeout(orbitLoaderHideTimer);
  const overlay = ensureOrbitLoader();
  overlay.dataset.stage = stage;
  overlay.querySelector(".aa-orbit-loader-title").textContent = title;
  overlay.querySelector(".aa-orbit-loader-detail").textContent = detail;
  overlay.setAttribute("aria-hidden", "false");
  requestAnimationFrame(() => overlay.classList.add("is-visible"));
}

function updateOrbitLoader(stage, title, detail) {
  const overlay = ensureOrbitLoader();
  overlay.dataset.stage = stage || "working";
  if (title) overlay.querySelector(".aa-orbit-loader-title").textContent = title;
  if (detail) overlay.querySelector(".aa-orbit-loader-detail").textContent = detail;
}

function hideOrbitLoader(delay = 0) {
  clearTimeout(orbitLoaderHideTimer);
  orbitLoaderHideTimer = setTimeout(() => {
    const overlay = document.getElementById("aaOrbitLoader");
    if (!overlay) return;
...
function completeOrbitLoader(title = "Tamamlandı", detail = "Sonuçlar hazır.") {
  updateOrbitLoader("complete", title, detail);
  hideOrbitLoader(650);
}
...
analyzeButton
  ?.addEventListener(
    ...
    async () => {
      ...
      showOrbitLoader(
        "analyzing",
        "Belgeyi analiz ediyoruz",
        "Yörüngen Kalem PDF'yi okuyor, önemli akademik bölümleri ve kanıtları ayırıyor."
      );
      ...
      try {
        ...
        completeOrbitLoader(
          "Analiz tamamlandı",
          "Belgenin akademik yapısı ve kanıtları hazırlandı."
        );
...
async function findManualSource() {
  ...
  showOrbitLoader(
    "searching",
    "Kaynakları tarıyoruz",
    onlyYear
      ? `${year} yılına ait akademik kayıtlar taranıyor.`
      : "Yörüngen Kalem verdiğin bilgilerle en uygun akademik kayıtları arıyor."
  );
  ...
  try {
    ...
    completeOrbitLoader(
      sources.length ? "Kaynaklar bulundu" : "Tarama tamamlandı",
      sources.length
        ? `${sources.length} olası akademik kayıt hazır.`
        : "Bu bilgilerle eşleşen kayıt bulunamadı; manuel eklemeye devam edebilirsin."
    );
...
async function sendAssistantMessage() {
  ...
  showOrbitLoader(
    "thinking",
    "Kaynakları düşünüyoruz",
    "Yörüngen Kalem seçili kaynakları, araştırma çerçevesini ve notlarını ilişkilendiriyor."
  );
  ...
  try {
    ...
    completeOrbitLoader(
      "Yanıt hazır",
      "Kaynak tabanlı yanıt oluşturuldu."
    );
  } catch (error) {
    hideOrbitLoader();
